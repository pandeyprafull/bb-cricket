...
function fetchUserTeams(userId, matchKey, fantasyType, joinedTeams = '') {
    return new Promise(async(resolve, reject) => {
        ...
        let userTeams = await SQL.Football.getUserTeamDetails(`*`, where);
        let userTeamsArray = [];
...
const leaguePreview = function(options) {
    return new Promise(async(resolve, reject) => {
        ...
        let leagueDetails = await SQL.Football.checkLeagueAvailability(leagueId, matchKey, fantasyType, userId, 1, 1, 1);
        console.log("LeagueDetails >>>>", leagueDetails);
        leagueDetails = leagueDetails[0];
        let msg = __("league_not_exists");
        let templateLeague;
        if (!leagueDetails) {
            templateLeague = await SQL.Football.getLeagueDetails(` where template_id = ${leagueId} and match_key = ${matchKey}`);
            templateLeague = templateLeague[0]
            leagueId = templateLeague.league_id
            let templateLeagueDetails = await SQL.Football.checkLeagueAvailability(leagueId, matchKey, fantasyType, userId, 1, 1, 1);
         ...
         * Validate all teams
         */
        var userDBTeams = await SQL.Football.getUserTeams(userId, matchKey, fantasyType, teams);
        if (!userDBTeams) {
            return reject({
                status: false,
        ...
        if (leagueDetails.is_infinity) {
            if (leagueDetails.total_joined >= Config.INFINITY_TERMINATOR) {
                await SQL.Football.updateLeague(`is_full=1`, `league_id=${leagueId}`, 'fb_leagues')
                return reject({
                    status: false,
                    code: Utils.StatusCodes.Error,
                    message: __("LEAGUE_FULL"),
                    ...
                    if (err) {
                        ...
                        if (userCurrentBal.unused_amount < userUnusedAmount || userCurrentBal.credits < userCreditsAmount || userCurrentBal.bonus_cash < userBonusAmount) {
                            ...
                            Bluebird.each(teamsArray, (element, index) => {
                                ...
                                var result = false;
                                try {
                                    result = await SQL.Football.userLeagueBulkInsert(leagueBulkData)
                                } catch (error) {
                                    connection.release()
                                    reject(error)
                                }
                                if (result && result.affectedRows) {
                                    result = await SQL.Football.creditstatsBulkInsert(creditStatsBulkData)
...
async function addChildLeague(matchKey, leagueId) {
    ...
    return new Promise(async(resolve, reject) => {
        let leagueDetails = await SQL.Football.getLeaguesDetails(matchKey, leagueId, "bb_leagues");
        // console.log("league Details>>>>>....", leagueDetails)
        if (leagueDetails == undefined || leagueDetails.length < 0) {
            leagueDetails = await SQL.Football.getLeaguesByTemplate(matchKey, leagueId)
        }
        leagueDetails = leagueDetails[0]
        ...
        if (leagueDetails.is_infinity) {
            ...
            if (leagueDetails.league_winner_type == 'fixed_winner') {
                ...
                await SQL.Football.updateLeague(setQuery, `league_id=${leagueId}`, 'fb_leagues')
            } else {
                ...
                if (currentWinners) {
                    ...
                    await SQL.Football.updateLeague(setQuery, `league_id=${leagueId}`, 'bb_leagues')
                }
            }
        }
        // update is full
        ...
        if (referenceLeagueId != 0) {
            referenceLeague = await SQL.Football.getLeaguesDetails(matchKey, referenceLeagueId, "fb_leagues");
        ...
        let newInsertedLeague = await SQL.Users.insertData(newLeagueData, "fb_leagues")
        ...
        await SQL.Football.leagueDatabulkInsert(winnerBulkData, "fb_leagues_data")
            //TODO: error handline if any error remove the added leagues
        await SQL.Football.updateLeague(`repeat_counts=repeat_counts+1`, `league_id=${leagueId}`, 'fb_leagues')
        await SQL.Football.updateLeague(`league_status=1`, `league_id=${newLeagueId}`, 'fb_leagues')
        return resolve({
            status: true,
            newInsertedLeague: newInsertedLeague
        })
...
module.exports = {
    ...
    getMatchV1Category: async(req, res, next) => {
        ...
        try {
            ...
            const selectedMatch = await SQL.Football.getMatchKey(matchKey, matchColumns, true);
           ...
           `,
            ...
            if (!teamCount) {
                teamCount = await SQL.Football.getTeamCountByMatch(user_id, matchKey, fantasyType);
            ...
            if (!matchLeagues) {
                ...
                matchLeagues = await SQL.Football.getEachActiveMatchesLeagues(matchKey, fantasyType);
            ...
            if (!joinedLeagues) {
                joinedLeagues = await SQL.Football.getJoinedLeagues(matchKey, user_id);
    ...
    leaguByCode: async(req, res) => {
        ...
        let leagueDetails = await SQL.Football.getLeagueDetails(`where league_code = '${leagueCode}'`)
        ...
        let matchDetails = await SQL.Football.getMatchByKey("*, if((start_date_unix-UNIX_TIMESTAMP()-closing_ts)<=0, 1, 0) as closed ", matchKey);
        ...
        console.log("matchhDetails >>>", matchDetails)
        ...
        let winners = await SQL.Football.getLeagueData(` league_id=${leagueId} `)
