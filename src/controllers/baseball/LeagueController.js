                    ...
                    if (err) {
                        ...
                        if (userCurrentBal.unused_amount < userUnusedAmount || userCurrentBal.credits < userCreditsAmount || userCurrentBal.bonus_cash < userBonusAmount) {
                            ...
                            Bluebird.each(teamsArray, (element, index) => {
                                let ticketCount = 0;
                                if (tickets.length > 0) ticketCount = tickets.length;
                                console.log("Inside ticketApplied", ticketApplied, index, ticketCount);
                                ...
                                var result = false;
                                try {
                                    result = await SQL.BaseBall.userLeagueBulkInsert(leagueBulkData)
                                } catch (error) {
                                    connection.release()
                                    reject(error)
                                }
                                if (result && result.affectedRows) {
                                    result = await SQL.Cricket.creditstatsBulkInsert(creditStatsBulkData)
                                    ...
                                    connection.commit();
                                    connection.release()
                                    console.log("result in insertLeague as fu*k>>>>>>",result)
                                    resolve(result)
                                } else {
                                    connection.release()
...
async function addChildLeague(matchKey, leagueId) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        let leagueDetails = await SQL.BaseBall.getLeaguesDetails(matchKey, leagueId, "bs_leagues");
        console.log("league Details>>>>>....", leagueDetails)
        if (leagueDetails == undefined || leagueDetails.length < 0) {
            leagueDetails = await SQL.BaseBall.getLeaguesByTemplate(matchKey, leagueId)
        }
        ...
        if (leagueDetails.is_infinity) {
            ...
            if (leagueDetails.league_winner_type == 'fixed_winner') {
                ...
                await SQL.BaseBall.updateLeague(setQuery, `league_id=${leagueId}`, 'bs_leagues')
            } else {
        ...
        if (!fullUpdated && (currentJoined >= maxTeams) && !leagueDetails.is_full) {
            await SQL.BaseBall.updateLeague(`is_full=1`, `league_id=${leagueId}`, 'bs_leagues')
        }
        ...
        if (referenceLeagueId != 0) {
            referenceLeague = await SQL.BaseBall.getLeaguesDetails(matchKey, referenceLeagueId, "bs_leagues");
            ...
            activeRefrenceLeague = await SQL.BaseBall.getLeagueDetails(`where reference_league=${referenceLeagueId} and max_players>total_joined`);
        }
        ...
        if (leagueDetails.total_winners <= 1) {
            ...
            winners = await SQL.BaseBall.getLeagueData(`league_id=${referenceLeagueId}`)
        } else {
            newLeagueData.league_status = 2;
            proceedLeagueData = true;
            winners = await SQL.BaseBall.getLeagueData(`league_id=${referenceLeagueId}`)
        }
        let newInsertedLeague = await SQL.BaseBall.insertData2(newLeagueData, "bs_leagues")
        ...
        if (!proceedLeagueData) {
            await SQL.BaseBall.updateLeague(`repeat_counts=repeat_counts+1`, `league_id=${leagueId}`, 'bs_leagues')
            return resolve({
                status: true,
        ...
        await SQL.BaseBall.leagueDatabulkInsert(winnerBulkData, "bs_leagues_data")
        //TODO: error handline if any error remove the added leagues
        await SQL.BaseBall.updateLeague(`repeat_counts=repeat_counts+1`, `league_id=${leagueId}`, 'bs_leagues')
        await SQL.BaseBall.updateLeague(`league_status=1`, `league_id=${newLeagueId}`, 'bs_leagues')
        return resolve({
            status: true,
            newInsertedLeague: newInsertedLeague
        })
...
function fetchUserTeams(userId, matchKey, fantasyType, joinedTeams = '') {
    return new Promise(async (resolve, reject) => {
        ...
        let userTeams = await SQL.BaseBall.getUserTeamDetails(`*`, where);
        let userTeamsArray = [];
...
const leaguePreview = function (options) {
    return new Promise(async (resolve, reject) => {
        ...
        let leagueDetails = await SQL.BaseBall.checkLeagueAvailability(userId, leagueId, matchKey, fantasyType, 1, 1, 1);
        leagueDetails = leagueDetails[0];
        let msg = __("LEAGUE_NOT_EXIST");
        let templateLeague;
        if (!leagueDetails) {
            templateLeague = await SQL.BaseBall.getLeagueDetails(` where template_id = ${leagueId} and match_key = ${matchKey} `);
            ...
            let templateLeagueDetails = await SQL.BaseBall.checkLeagueAvailability(userId, leagueId, matchKey, fantasyType, 1, 1, 1);
         ...
         * Validate all teams
         */
        var userDBTeams = await SQL.BaseBall.getUserTeams(userId, matchKey, fantasyType, teams);
        if (!userDBTeams) {
            return reject({
                status: false,
...
module.exports = {
                    ...
                    insertLeagueJoiningData(userDetails, leagueDetails, teamsArray, userUnusedAmount, userCreditsAmount, userBonusAmount, { unusedAmntApplied: unusedAmntApplied, creditsApplied: creditsApplied, bonusAmtApplied: bonusAmtApplied }, success.response.ticketApplied, ticket, ip).then(async result => {
                        ...
                        if (result.affectedRows) {
                            ...
                            if (leagueDetails.league_type == 1) { // if not practice and freerolls league
                                let userTickets = false;
                                userTickets =  ticket.length > 0 ? ticket[0] : false;
                                if (!userTickets) {
                                ...
                                // var query = `UPDATE bb_users SET total_contest_joined = total_contest_joined+1, last_contest_date ='${currentDate}',modified_date ='${currentDate}', WHERE user_id=${userId}`
                            ...
                            await addChildLeague(matchKey, leagueId).then(result =>{
                                console.log("result in addchild League", result);
                            }).catch(err => console.log("err in addChild League", err)
                            )
    ...
    getMatchV1Category: async (req, res, next) => {
        ...
        try {
            ...
            const selectedMatch = await SQL.BaseBall.getMatchKey(matchKey, matchColumns, true);
            console.log("selected Match is :::", selectedMatch);
            ...
            let catagoryLeague = await SQL.BaseBall.getLeagueByCatagory(
           ...
           `,
            ...
            if (!teamCount) {
                teamCount = await SQL.BaseBall.getTeamCountByMatch(user_id, matchKey, fantasyType);
            ...
            //Match Leagues
            let matchLeagues = await Utils.Redis.getAsync(Utils.Keys.BASEBALL_LEAGUES + '_' + fantasyType + "_" + matchKey);
            if (!matchLeagues) {
                console.log("Matchleagues From DB");
                matchLeagues = await SQL.BaseBall.getEachActiveMatchesLeagues(matchKey, fantasyType);
                await Utils.Redis.set(Utils.Keys.BASEBALL_LEAGUES + '_' + fantasyType + "_" + matchKey, JSON.stringify(matchLeagues), 'EX', Utils.RedisExpire.BASEBALL_LEAGUES);
            } else {
                matchLeagues = JSON.parse(matchLeagues);
            }
            //joinedLeagues
            let joinedLeagues = await Utils.Redis.getAsync(Utils.Keys.BASEBALL_JOINED_LEAGUES + user_id + matchKey);
            if (!joinedLeagues) {
                joinedLeagues = await SQL.BaseBall.getJoinedLeagues(matchKey, user_id);
                await Utils.Redis.set(Utils.Keys.JOINED_LEAGUES + user_id + matchKey, JSON.stringify(joinedLeagues), 'EX', Utils.RedisExpire.BASEBALL_JOINED_LEAGUES)
            } else {
    ...
    leagueByCode: async (req, res) => {
        ...
        let leagueDetails = await SQL.BaseBall.getLeagueDetails(`where league_code = '${leagueCode}'`)
        ...
        let matchDetails = await SQL.BaseBall.getMatchByKey(`*, if((start_date_unix-UNIX_TIMESTAMP()-closing_ts)<=0, 1, 0) as closed `, matchKey);
        ...
        let winners = await SQL.BaseBall.getLeagueData(`league_id=${leagueId}`)
