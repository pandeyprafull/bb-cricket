const db = require('../utils/basketBallDbConfig');
...
module.exports = {
    ...
    FROM
        bk_matches AS t1
            LEFT JOIN
        bk_live_scores AS t1_1 ON t1.match_key = t1_1.match_key
            LEFT JOIN
        bk_teams AS t2 ON t1.team_a_key = t2.team_key
            LEFT JOIN
        bk_teams AS t3 ON t1.team_b_key = t3.team_key
    WHERE
    ...
    getHomoPromoBanners: async () => {
        ...
        return await db.query(sql, [1, 6]);
    },
    getMatchByKey: async (columns = "*", matchKey) => {
        const sql = `select ${columns} from bk_matches as t1
        LEFT JOIN
        bk_teams AS t2 ON t1.team_a_key = t2.team_key
        LEFT JOIN
        bk_teams AS t3 ON t1.team_b_key = t3.team_key where match_key = ?`;
    ...
    getJoinedLeagues: async (matchKey, user_id) => {
        let sql = `SELECT
                ...
                FROM
                    bk_user_leagues AS t1 left join bk_leagues AS t2 on t1.league_id = t2.league_id
                WHERE
                    t1.match_key = ?
                        AND t1.user_id = ? `;
    ...
    getMatchPlayers: async (matchKey) => {
        let sql = `SELECT
                    ...
                    FROM
                        bk_season_players AS t1
                            LEFT JOIN
                        bk_seasonal_points AS t2 ON t1.season_key = t2.season_key
    ...
    getPlayerSelections: async (seasonKey, playerKey, matchKey = false) => {
        ...
        let query = `select ${columns} from bk_player_selections as t1 left join bk_matches as t2
    ...
    getMatchTableByKey: async (matchKey, columns = "*") => {
        let sql = `select *, if((start_date_unix-UNIX_TIMESTAMP()-closing_ts)<=0, 1, 0) as closed from bk_matches where match_key = ${matchKey}`
        if (columns) {
            sql = `select ${columns}, if((start_date_unix-UNIX_TIMESTAMP()-closing_ts)<=0, 1, 0) as closed from bk_matches where match_key = ${matchKey}`
        }
    ...
    getPlayersByMatchKey: async (where, columns = "*", IF = null) => {
        const sql = `SELECT ${columns} from bk_season_players where ${where}`
        // console.log('sql=>   ', sql);
    ...
    getUserTeamDetails: async (columns = `*`, where) => {
        ...
        const sql = `SELECT
                    ...
                    FROM
                        bk_user_teams AS t1
                            LEFT JOIN
                        bk_season_players AS t2 ON t1.match_key = t2.match_key
                            AND t1.player_key = t2.player_key
                    WHERE
                        ${where}
    ...
    removeUserTeam: async (where) => {
        const sql = `DELETE FROM bk_user_teams where ${where}`;
        const result = await db.query(sql);
    ...
    checkLeagueAvailability: async (userId, leagueId, matchKey, fantasyType, leagueStatus, adminStatus, active) => {
        ...
        query = `SELECT
                    ...
                    FROM
                        bk_leagues AS t1
                            LEFT JOIN
                        bk_matches AS t2 ON t1.match_key = t2.match_key
                            LEFT JOIN
                        bk_teams AS t3 ON t2.team_a_key = t3.team_key
                            LEFT JOIN
                        bk_teams AS t4 ON t2.team_b_key = t4.team_key
                            LEFT JOIN
                        bk_user_leagues AS t5 ON t1.match_key = t5.match_key
    ...
    checkTeamExists: async (userId, matchKey, fantasyType, teamNumber, table = "bk_user_teams") => {
    ...
    swapTeam: async (matchKey, leagueId, userId, oldTeam, newTeam, userIp = false) => {
        ...
        let query = ` update bk_user_leagues set ${update} where user_id = ${userId} and match_key = ${matchKey} and league_id = ${leagueId} and team_number = '${oldTeam}' `;
    ...
    getUserLeaguesWithTopRank: async (userId = null, matchKey, table = "bk_user_leagues", league_id = null, fantasyType = 0) => {
        let sql;
        let values;
        if (userId) {
            sql = ` SELECT
...
WHERE
    ...
    getUserLeaguesDetails: async (matchKey, leagueId, table = "bk_user_leagues") => {
    ...
    getLeagueDetails: async (where) => {
        const sql = `select * from bk_leagues ${where}`;
    ...
    getLeaguesDetails: async (matchKey, leagueId) => {
        let sql = `select * from bk_leagues where match_key=? and league_id=?`;
    ...
    getLeaguesByTemplate: async (matchKey, leagueId) => {
        let sql = `select * from bk_leagues where match_key=? and template_id=?`;
        let result = await db.query(sql, [matchKey, leagueId]);
        console.log("Results ......", result);
        return result = result.length > 0 ? result : false
    },
    getLeagueData: async (where, columns = '*') => {
        let sql = `select ${columns} from bk_leagues_data where ${where}`
    ...
    userLeagueBulkInsert: async (data) => {
        ...
        let columns = Object.keys(data);
        let sql = `insert into bk_user_leagues (${columns}) VALUES ?`;
    ...
    removeLeagueIfError: async (league_id) => {
        const sql = ` DELETE FROM bk_leagues where league_id = ? `;
    ...
    removeDataLeaguesDatatable: async (league_id) => {
        const sql = `DELETE FROM bk_leagues_data where league_id = ? `;
        const result = await db.query(sql, [league_id]);
        return result;
    },
    updateLeagueStatus: async (leagues_id, status) => {
        const sql = `UPDATE bk_leagues SET league_status = ? where league_id = ?`;
        const result = await db.query(sql, [status, leagues_id]);
        return result;
    },
    getMatchfantasyScore: async (matchKey) => {
        let sql = `select t1.* , t2.team_key as team_a_key_basket, t2.team_flag as team_a_flag, t3.team_key as team_b_key_basket, t3.team_flag as team_b_flag , if((start_date_unix-UNIX_TIMESTAMP()-closing_ts)<=0, 1, 0) as closed from bk_matches as t1 left join bk_teams as t2 on t1.team_a_key=t2.team_key left join bk_teams as t3 on t1.team_b_key=t3.team_key where t1.match_key = ? order by t1.start_date_unix ASC`;

        return await db.query(sql, [matchKey]);

    },
    getPlaying22ByUserTeam: async (matchKey, userId, teamNumber, fantasyType, readTable = "bk_user_teams") => {
        ...
        let sql = `SELECT
                    ...
                    FROM
                        ${readTable} AS t1
                        LEFT JOIN
                    bk_playing22 AS t2 ON t1.match_key = t2.match_key
                        AND t1.player_key = t2.player_key
                        LEFT JOIN
                    bk_season_players AS t4 ON t1.match_key = t4.match_key
    ...
    getLeagueByCatagory: async (where, userId) => {
        let sql = `SELECT
                ...
                FROM
                    bk_leagues AS t1
                        LEFT JOIN
                    bk_user_leagues AS t2 ON t1.match_key = t2.match_key
